...
优化改进 v3：
- 引用独立模块（models, utils, services, core）
- 消除重复代码
- 统一 API Key 获取
- 更清晰的架构分层
"""
...
# 引用新模块
from models.task import Task, WorkflowResult
from models.agent import AsyncSubAgent
from services.agent_factory import create_agents_from_config, get_api_config
from services.llm_client import LLMClient
from utils.knowledge_parser import parse_knowledge, fix_json_quotes
from utils.prompt_builder import build_keypoint_question, build_topic_question, build_relation_question
from utils.logger import setup_logger
from core.knowledge_utils import count_keypoints, has_practice_project, has_interview_highlights
...
请生成完整的知识架构 YAML。"""
    ...
    def initialize(self):
        ...
        self.agents = create_agents_from_config(self.config, self.enable_cache)
        self._create_tasks()
    ...
    def _build_question(self, topic_name: str, layer_num: int, previous_layer_topics: List[str] = None) -> str:
        """构建提问（引用统一模块）"""
        return build_topic_question(topic_name, layer_num, previous_layer_topics)

    def _build_keypoint_question(
        self, topic_name: str, subtopic_name: str, key_point: str, difficulty: str, layer_num: int = 3
    ) -> str:
        """构建提问（引用统一模块）"""
        return build_keypoint_question(topic_name, subtopic_name, key_point, difficulty, layer_num)
    ...
    def _build_relation_question(self, topic_name: str, knowledge: Dict, layer_num: int) -> str:
        """构建提问（引用统一模块）"""
        return build_relation_question(topic_name, knowledge, layer_num)
    ...
    def _parse_knowledge(self, content: str, topic_name: str) -> Dict:
        """解析知识内容（引用统一模块）"""
        return parse_knowledge(content, topic_name)

    def _fix_json_quotes(self, raw_json: str) -> str:
        """修复 JSON（引用统一模块）"""
        return fix_json_quotes(raw_json)
